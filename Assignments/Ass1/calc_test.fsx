...
(* Helper function to perform the iteration and accumulation within the OVER case. *) 
let rec evaluateRange (current : int) (endValue : int) (acc : int) (combineValues : int -> int -> int) (var : string) (e : EXP) (vtab : SymTab) (eval : SymTab -> EXP -> VALUE) : VALUE =
    if current > endValue then
        INT acc
    else
        let new_vtab = bind var (INT current) vtab
        let value = eval new_vtab e
        match value with
        | INT n -> evaluateRange (current + 1) endValue (combineValues acc n) combineValues var e vtab eval
...
let rec eval (vtab : SymTab) (e : EXP) : VALUE =
    ...
    // let combineValues acc n =
    //     match rop with
    //     | RSUM -> INT (acc + n)
    //     | RPROD -> INT (acc * n)
    //     | RMAX -> INT (max (fst acc) n)
    //     | RARGMAX -> if n > fst acc then INT n else acc

    // match (eval vtab e1, eval vtab e2) with
    // | INT startValue, INT endValue ->
    //     let initialAcc = (System.Int32.MinValue, INT System.Int32.MinValue)
    //     let maxValue = evaluateRange startValue endValue initialAcc combineValues var e3 vtab eval
    //     snd maxValue
    
    // Below the latest version with the helper, but it has errors.
    // | OVER (rop, var, e1, e2, e3) ->
    // let combineValues acc n =
    //     match rop with
    //     | RSUM -> INT (acc + n)
    ...
    //     | RARGMAX ->
    //          match acc with
    //          | (INT maxValue, INT argValue) ->
    //          if n > maxValue then (INT n, INT n) else (INT maxValue, INT argValue)
    //          | _ -> failwith "Invalid accumulator type in RARGMAX"


    // match (eval vtab e1, eval vtab e2) with
    // | INT startValue, INT endValue ->
    //     let initialAcc = (INT System.Int32.MinValue, INT System.Int32.MinValue)
    //     let maxValue = evaluateRange startValue endValue initialAcc combineValues var e3 vtab eval
    //     snd maxValue
    ...
    //     | RPROD -> acc * n
    //     | RMAX -> max acc n
    //     | RARGMAX -> if n > fst (acc : VALUE) then INT n else acc

    // match (eval vtab e1, eval vtab e2) with
    // | INT startValue, INT endValue ->
    //     let initialAcc = INT System.Int32.MinValue
    //     let maxValue = evaluateRange startValue endValue initialAcc combineValues var e3 vtab eval
    //     maxValue

    
    // | OVER (rop, var, e1, e2, e3) ->
    // let value1 = eval vtab e1
    // let value2 = eval vtab e2
    // match rop with
    // | RSUM ->
    ...
    //             match value with
    //             | INT n -> sumRange (current + 1) endValue (acc + n)
    ...
    // | RMAX ->
    //     let rec maxRange (current : int) (endValue : int) (acc : int) =
    //         if current > endValue then
    //             INT acc
    //         else
    //             let new_vtab = bind var (INT current) vtab
    //             let value = eval new_vtab e3
    //             match value with
    //             | INT n -> maxRange (current + 1) endValue (max acc n)
              
    //     match value1, value2 with
    //     | INT startValue, INT endValue ->
    //         maxRange startValue endValue System.Int32.MinValue
     

    // | RARGMAX ->
    //     let rec argMaxRange (current : int) (endValue : int) (argMax : int) (maxValue : int) =
    //         if current > endValue then
    //             INT argMax
    //         else
    //             let new_vtab = bind var (INT current) vtab
    ...
    //             | INT n ->
    //                 if n > maxValue then
    //                     argMaxRange (current + 1) endValue current n
    //                 else
    ...
    //     let value1 = eval vtab e1
    //     let value2 = eval vtab e2
    //     let value3 = eval vtab e3
    //     match rop with
    //     | RSUM ->  // Perform the desired behavior for RSUM
    //         // Example: Return the sum of the evaluated values
    //         match (value1, value2, value3) with
    //         | INT n1, INT n2, INT n3 -> INT (n1 + n2 + n3)
    //     | RPROD -> // Perform the desired behavior for RPROD
    //         // Example: Return the product of the evaluated values
    //         match (value1, value2, value3) with
    //         | INT n1, INT n2, INT n3 -> INT (n1 * n2 * n3)
    //     | RMAX ->  // Perform the desired behavior for RMAX
    //         // Example: Return the maximum of the evaluated values
    //         match (value1, value2, value3) with
    //         | INT n1, INT n2, INT n3 -> INT (max n1 (max n2 n3))
    //     | RARGMAX -> // Perform the desired behavior for RARGMAX
    //         // Example: Return the argument with the maximum value
    //         match (value1, value2, value3) with
    //         | INT n1, INT n2, INT n3 ->
    //             let maxValue = max n1 (max n2 n3)
    //             if maxValue = n1 then INT n1
    //             elif maxValue = n2 then INT n2
    //             else INT n3 
